...
class AssignedIssueConsumer(AsyncWebsocketConsumer):
    ...
    @sync_to_async
    def update_issue_status_db(self, issue_id, status):
        """Update issue in DB"""
        issue = Issue.objects.get(id=issue_id, assigned_to=self.user)
        issue.status = status
        issue.save()
    ...
    async def update_issue_status(self, issue_id, status):
        try:
            issue = await self.update_issue_status_db(issue_id, status)

            # Broadcast update to user group
    ...
    @sync_to_async
    def delete_issue_db(self, issue_id):
        """Delete issue in DB"""
        issue = Issue.objects.get(id=issue_id, assigned_to=self.user)
        issue.delete()
        return issue_id
    ...
    async def delete_issue(self, issue_id):
        try:
            deleted_id = await self.delete_issue_db(issue_id)
